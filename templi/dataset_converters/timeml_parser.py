...
def parser(news, sentences_in_one_data=1, return_only_sentences_anno=False):
    ...
    tags_pos = list(filter(lambda x: x[0][1] is not None, tags_pos))
    ...
    sent_pos = [
        (
            "".join(
                map(
                    lambda x: x[0],
                    sent_pos[i : i + sentences_in_one_data],
                )
            ),
            sent_pos[i][1],
        )
    ...
    sentences_anno = {}
    for absolute_idx_of_sentence in sent_pos:
        ...
        def spaceless_rng(idx_of_tag):
            startidx = len(
                sentence[: idx_of_tag[1] - abs_startidx + 1].replace(" ", "")
            )
            endidx = len(
                sentence[
                    : idx_of_tag[1] + len(idx_of_tag[0][0]) - abs_startidx + 1
                ].replace(" ", "")
            )
            return (startidx, endidx)

        sentences_anno[sentence] = {
            spaceless_rng(i): i[0][1]
            for i in tags_pos
            if abs_startidx <= i[1] <= abs_endidx
        }
    ...
    events_list = parsed.xpath("//EVENT")
