...
class BertForTokenClassification(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, return_dict=True):
        sequence_output, _ = self.bert(
            input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False
        )
...
def seperate_seq_attention(attention_mask, doc_len, ques_len):
    ...
    for i in range(doc_len.size(0)):
        ...
        ques_seq_output[i, : ques_len[i]] = attention_mask[
            i, doc_len[i] + 2 : doc_len[i] + ques_len[i] + 2
        ]

        doc_ques_seq_output[i, : doc_len[i]] = attention_mask[i, 1 : 1 + doc_len[i]]
        doc_ques_seq_output[i, doc_len[i] : doc_len[i] + ques_len[i]] = attention_mask[
            i, doc_len[i] + 2 : doc_len[i] + ques_len[i] + 2
        ]
...
def seperate_seq(sequence_output, doc_len, ques_len):
    ...
    for i in range(doc_len.size(0)):
        ...
        ques_seq_output[i, : ques_len[i]] = sequence_output[
            i, doc_len[i] + 2 : doc_len[i] + ques_len[i] + 2
        ]
        doc_ques_seq_output[i, : doc_len[i]] = sequence_output[i, 1 : 1 + doc_len[i]]
        doc_ques_seq_output[i, doc_len[i] : doc_len[i] + ques_len[i]] = sequence_output[
            i, doc_len[i] + 2 : doc_len[i] + ques_len[i] + 2
        ]
...
class BertMultiwayMatch(PreTrainedBertModel):
    ...
    def forward(
        ...
        passage_encoded, question_encoded, passage_question_encoded = seperate_seq(
            sequence_output, doc_len, ques_len
        )
        (
            passage_attention_mask,
            question_attention_mask,
            passage_question_attention_mask,
        ) = seperate_seq_attention(flat_attention_mask, doc_len, ques_len)
...
class BertEventAttentive(PreTrainedBertModel):
    ...
    max_seq_length needs to be defined first
    """
    ...
    def __init__(self, config, max_seq_length=128):
        ...
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.attn = nn.Linear(config.hidden_size, max_seq_length)
        self.apply(self.init_bert_weights)

    def match_attn(self, doc_len, attn_output):
        # -inf out all values which do not correspond to a token
        for i in range(doc_len.size(0)):
            attn_output[i, doc_len[i]:].fill_(float("-inf"))
        return attn_output
    ...
    def forward(
        ...
        attn_weights = F.softmax(
            self.match_attn(doc_len, self.attn(pooled_output)), dim=1
        )  # (16, 128)
