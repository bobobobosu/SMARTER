...
class TempliSemanticParser(Model):
    """
    A ``TempliSemanticParser`` is a :class:`Model` which takes as input a sentence and a main_var,
    and produces a logical form that evaluates to all the relationship between main_var and temp_vars.
    The logical form is generated by a `type-constrained`, `transition-based` parser. This is an
    ...
    ``TemliErmSemanticParser``, or if you have a set of approximate logical forms that give the
    correct denotation, you will want to use ``TemliMmlSemanticParser``.
    ...
    def _get_initial_rnn_and_grammar_state(
        ...
        if self._use_neighbor_similarity_for_linking:
            ...
            linking_scores = (
                projected_question_entity_similarity
        ...
        initial_grammar_state = [
            self._create_grammar_state(
                world[i], actions[i], linking_scores[i], entity_types[i]
            )
            for i in range(batch_size)
        ]
    ...
    def _get_neighbor_indices(
        worlds: List[TempliLanguage], num_entities: int, tensor: torch.Tensor
    ) -> torch.LongTensor:
        """
    ...
    def _action_history_match(predicted: List[int], targets: torch.LongTensor) -> int:
        ...
        return torch.max(
            torch.min(targets_trimmed.eq(predicted_tensor), dim=1)[0]
        ).item()
        ...
        for i in range(batch_size):
            ...
            if i in best_final_states:
                ...
                for action_indices in all_action_indices:
                    ...
                    try:
                        logical_form = world[i].action_sequence_to_logical_form(
                            action_strings
                        )
